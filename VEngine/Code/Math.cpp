...
void Math::MatrixAddScale(float s, XMMATRIX& m)
{
    m.r[0].m128_f32[0] = s;
    m.r[1].m128_f32[1] = s;
    m.r[2].m128_f32[2] = s;
...
XMVECTOR Math::XMVectorRight()
{
    return XMVectorSet(1.f, 0.f, 0.f, 0.f);
}

XMVECTOR Math::XMVectorUp()
{
    return XMVectorSet(0.f, 1.f, 0.f, 0.f);
}

XMVECTOR Math::XMVectorForward()
{
...
XMFLOAT3 Math::PitchYawRollFromMatrix(XMMATRIX m)
{
...
XMFLOAT3 Math::PitchYawRollFromQuaternion(XMFLOAT4 q)
{
    XMMATRIX m = XMMatrixRotationQuaternion(XMLoadFloat4(&q));
    return XMFLOAT3(PitchYawRollFromMatrix(m));
}

void Math::LookAtRotation(XMVECTOR lookAtPoint, XMMATRIX& m)
{
...
XMFLOAT4X4 Math::FbxMatrixToDirectXMathMatrix(fbxsdk::FbxMatrix fbxMatrix)
{
...
bool Math::VecEqual(XMVECTOR v1, XMVECTOR v2, float epsilon)
{
...
bool Math::Float3Equal(XMFLOAT3& f1, XMFLOAT3& f2)
{
...
float Math::FindMaxInVector(XMVECTOR v)
{
    float max = 0.f;
    for (int i = 0; i < 4; i++)
    {
...
XMVECTOR Math::XMVectorConstantLerp(XMVECTOR current, XMVECTOR target, float dist)
{
...
XMMATRIX Math::GetBoundingBoxMatrix(BoundingOrientedBox& boundingBox, Actor* actor)
{
...
void Math::UpdateBoundingBox(BoundingOrientedBox& boundingBox, Actor* actor)
{
    //XMFLOAT3 actorPosFloat3 = actor->GetPositionFloat3();
    //XMVECTOR actorPos = XMLoadFloat3(&actorPosFloat3);
    //XMVECTOR boundingBoxCenter = XMLoadFloat3(&boundingBox.Center);
